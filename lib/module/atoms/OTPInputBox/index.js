import { StyleSheet, View, Pressable, Platform, Keyboard } from 'react-native';
...
import { getBorderColor } from './config';
const OTPInputBoxComponent = /*#__PURE__*/React.forwardRef((props, ref) => {
  ...
  const [currentFocusedIndex, setFocusedIndex] = useState(0);
  const {
    colors
  } = useTheme();
  ...
  const refs = useRef(new Array(maxOTPSize).fill(undefined).map(() => /*#__PURE__*/React.createRef()));
  const otpString = useMemo(() => {
    var _otp$filter;
  ...
  const styles = useMemo(() => _styles(colors), [colors]);
  return /*#__PURE__*/React.createElement(View, null, /*#__PURE__*/React.createElement(Pressable, {
    ...
    }, /*#__PURE__*/React.createElement(BaseTextInput, {
      ref: ref,
      textAlign: "center",
      style: styles.boxStyleWrapper,
      textInputStyle: [styles.boxStyle, styles.textInputColor, TextVariants[TextTokenV3.semibold_500]],
      autoFocus: index === 0,
      clearTextOnFocus: true,
      placeholderTextColor: colors.text.defaultTertiary,
      textContentType: "oneTimeCode",
      borderColor: getBorderColor(!!showFocused, !!showError, colors),
...
const OTPInputBox = /*#__PURE__*/React.memo(OTPInputBoxComponent);
export { OTPInputBox };
const _styles = colors => StyleSheet.create({
  hint: {
    marginTop: Sizes.Space.space4
  },
  otpContainer: {
    flexDirection: 'row',
    width: '100%'
  ...
  textInputColor: {
    color: colors.text.defaultPrimary
  }
});
